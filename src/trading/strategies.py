...
class HMMStrategy(BaseStrategy):
    ...
    """
    def __init__(self, n_components: int = 3, model_order: int = 1, optimize_order: bool = False, max_order_to_test: int = 10, ranking_metric: str = 'sharpe', retrain_max_age_days: int = 30, walk_forward_window: int = 252, retrain_period: int = 63, use_causality_filter: bool = False, causality_significance: float = 0.05, use_causal_features: bool = True, causal_dag_file: str = None, optimize_n_components: bool = True, n_components_range: tuple = (2, 4)):
        """
        Initializes the HMM-based strategy.

        Args:
            n_components (int): The number of hidden states for the HMM (default 3 for multi-regime classification).
            ...
            use_causality_filter (bool): If True, filter features based on quantile Granger causality.
            causality_significance (float): P-value threshold for causality tests (default 0.05).
            use_causal_features (bool): If True, use causal DAG features from parent stocks (default True).
            causal_dag_file (str): Path to the causal DAG pickle file (if None, uses default).
            optimize_n_components (bool): If True, use AIC/BIC to select optimal number of states (default True).
            n_components_range (tuple): Range of components to test (min, max) when optimizing (default (2, 4)).
        """
    ...
    def analyze(self, ticker: str, bars_data: pd.DataFrame, sp500_data: pd.DataFrame = None):
        """
        ...
        Args:
            ...
            sp500_data (pd.DataFrame, optional): Pre-fetched S&P 500 return data to optimize API calls.
        ...
        analyzer = AnalyzeHMM(
            ...
            causality_significance=self.causality_significance,
            use_causal_features=self.use_causal_features,
            causal_dag_file=self.causal_dag_file,
        ...
        # Now incorporate confidence into the ranking
        ...
        if self.ranking_metric == 'sharpe':
            ...
            # Weight Sharpe ratio by confidence
            prediction['ranking_strength'] = sharpe * confidence
        else: # 'return'
            # Weight expected return by confidence
            prediction['ranking_strength'] = prediction['predicted_state_mean_return'] * confidence
        
        # Add position sizing information to prediction
        position_size = prediction.get('position_size', 0.0)
        position_action = prediction.get('position_action', 'hold')
        
        prediction['position_size'] = position_size
        prediction['position_action'] = position_action
    ...
    def generate_signals(self, bars_data: pd.DataFrame):
        ...
        for i in range(self.walk_forward_window, len(bars_data), self.retrain_period):
            ...
            # Handle multi-state regimes: Use top and bottom states
            # For 2 states: bear/bull
            # For 3 states: bear/neutral/bull -> only enter on bull, exit on bear
            # For 4 states: strong_bear/mild_bear/mild_bull/strong_bull -> enter on strong_bull, exit on strong_bear
            bearish_state = analyzer.state_regimes[0]  # Worst state (lowest return)
            bullish_state = analyzer.state_regimes[-1]  # Best state (highest return)
            
            # Entry: Only on most bullish state
            # Exit: Only on most bearish state
            # This creates a more conservative strategy that waits for strong signals
            entry_signals = (states == bullish_state).astype(bool)
            exit_signals = (states == bearish_state).astype(bool)
            
            entries.update(entry_signals)
