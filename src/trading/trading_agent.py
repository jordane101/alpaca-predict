...
from src.trading.strategies import BaseStrategy

def _worker_analyze_ticker(strategy: BaseStrategy, tradable_universe: list, ticker: str, bars_df: pd.DataFrame, is_held: bool, sp500_data: pd.DataFrame = None):
    """
    ...
    Args:
        ...
        sp500_data (pd.DataFrame, optional): Pre-fetched S&P 500 return data to avoid redundant API calls.

    Returns:
        tuple: A tuple containing (signal_type, data).
    """
    try:
        outlook, data = strategy.analyze(ticker, bars_df, sp500_data=sp500_data)
        position_action = data.get('position_action', 'hold')
        position_size = data.get('position_size', 0.0)
        confidence = data.get('confidence', 0.0)
        
        print(f"  -> Analysis result for {ticker}: Outlook={outlook}, Action={position_action}, "
              f"PosSize={position_size:+.2f}, Confidence={confidence:.2%}, Strength={data.get('ranking_strength', 'N/A')}")

        # Handle held positions
        if is_held:
            if outlook == 'negative' or position_action == 'sell':
        ...
        # Handle non-held positions
        if ticker not in tradable_universe:
            return 'no_action', ticker
        
        # Long positions (traditional buy)
        if outlook == 'positive' and position_action == 'buy' and position_size > 0:
            print(f"  -> BUY SIGNAL for {ticker}. Strength: {data['ranking_strength']:.4f}, Size: {position_size:.2%}")
            return 'positive', data
        
        # Short positions (negative/bearish with high confidence)
        elif position_action == 'short' and position_size < 0:
            print(f"  -> SHORT SIGNAL for {ticker}. Strength: {data['ranking_strength']:.4f}, Size: {position_size:.2%}")
            # Treat shorts as "positive" signals (actionable opportunities)
            return 'positive', data
...
class TradingAgent:
    ...
    def __init__(self, name: str, strategy: BaseStrategy, trading_client: TradingClient, data_client: StockHistoricalDataClient, max_positions: int = 10, total_allocation_pct: float = 0.5, waterfall_allocation_pcts: list = None, stop_loss_pct: float = None, take_profit_pct: float = None, max_analysis_workers: int = 8, asset_class: str = 'us_equity'):
        ...
        if self.asset_class == 'crypto':
            ...
            # Try to load DAG universe first, fall back to S&P 500 if not available
            self.tradable_tickers = self._get_dag_universe()
            print(f"Equity agent '{self.name}' initialized with {len(self.tradable_tickers)} tickers.")
    ...
    def _fetch_sp500_data(self):
        ...
        try:
            ...
            spy_bars = self.data_client.get_stock_bars(request_params)
            
            if spy_bars.df.empty:
                print("Warning: Could not fetch S&P 500 data. Analyses will proceed without it.")
                return None
            
            # Extract and prepare S&P 500 return data
            spy_df = spy_bars.df.reset_index(level='symbol', drop=True).copy()
            spy_df['SP500_Return'] = spy_df['close'].pct_change()
            
            # Return only the SP500_Return column
            return spy_df[['SP500_Return']].copy()
            
        except Exception as e:
            print(f"Error fetching S&P 500 data: {e}. Analyses will proceed without it.")
            return None
    ...
    def _scan_and_analyze_market(self, held_tickers):
        ...
        # Fetch S&P 500 data once for all stocks (optimization)
        sp500_data = self._fetch_sp500_data()
        if sp500_data is not None:
            print(f"Fetched S&P 500 data: {len(sp500_data)} rows (will be reused for all tickers)")
                ...
                with ProcessPoolExecutor(max_workers=num_workers) as executor:
                    ...
                    for ticker in batch:
                        if ticker in grouped_data.groups:
                            ...
                            future = executor.submit(_worker_analyze_ticker, self.strategy, self.tradable_tickers, ticker, ticker_df, is_held, sp500_data)
    ...
    def _decide_buys(self, positive_predictions, account, all_positions, owned_tickers):
        ...
        for i, pick in enumerate(top_picks):
            # Get base allocation from waterfall
            base_allocation_pct = self.waterfall_allocation_pcts[i]
            
            # Get confidence-based position size (0-1 for long, 0 to -1 for short)
            position_size = pick.get('position_size', 1.0)
            confidence = pick.get('confidence', 1.0)
            position_action = pick.get('position_action', 'buy')
            
            # Scale the base allocation by the recommended position size
            # position_size is already confidence-weighted (0-1 for longs, 0 to -1 for shorts)
            adjusted_allocation = base_allocation_pct * abs(position_size)
            
            # Calculate notional value
            notional_value = cash_for_new_buys * adjusted_allocation
            
            # For short positions, make notional negative
            if position_size < 0:
                notional_value = -notional_value
            
            pick['notional_value'] = round(notional_value, 2)
            pick['adjusted_allocation'] = adjusted_allocation
            
            action_str = "SHORT" if position_size < 0 else "BUY"
            print(f"  - {pick['ticker']}: Action={action_str}, Strength={pick['ranking_strength']:.4f}, "
                  f"Confidence={confidence:.2%}, PosSize={position_size:+.2%}, Notional=${pick['notional_value']:,.2f}")
            
            if abs(pick['notional_value']) >= 1:
    ...
    def _get_dag_universe(self):
        """Loads tickers from the Causal DAG universe (tech stocks)."""
        import pickle
        from pathlib import Path
        
        print("Loading tickers from Causal DAG...")
        try:
            # Try to find the DAG file
            project_root = Path(__file__).parent.parent.parent
            dag_file = project_root / "data" / "causality_cache" / "large_network_graph_dag.pkl"
            
            if not dag_file.exists():
                print(f"  DAG file not found at {dag_file}")
                print(f"  Falling back to S&P 500...")
    ...
    def _get_sp500_tickers(self):
        ...
        try:
            ...
            print(f"  Found {len(tickers)} tickers from S&P 500")
            return tickers
        except Exception as e:
            print(f"  Could not fetch S&P 500 tickers: {e}")
            print(f"  ⚠️  WARNING: No tickers available! Trading will not work.")
            return []